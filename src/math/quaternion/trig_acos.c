...
**	so we compute the complex principal value `acos(z) = -i*ln(iz + sqrt(1 - z^2)) + PI/2`
...
s_quaternion	Quaternion_ArcCos(s_quaternion const* q)
{
	...
	// acos(z) = PI/2 - asin(z) = PI/2 - (arg(w) - i*ln|w|)
	t_float	result_re = (t_float)PI * 0.5 - Float_ArcTan2(w_im, w_re);
