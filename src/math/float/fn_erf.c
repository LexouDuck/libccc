...
{ \
	t_f##BITS s,P,Q; \
	s = abs_x - 1; \
	P = pa_f##BITS[0]+s*(pa_f##BITS[1]+s*(pa_f##BITS[2]+s*(pa_f##BITS[3]+s*(pa_f##BITS[4]+s*(pa_f##BITS[5]+s*pa_f##BITS[6]))))); \
...
{ \
	...
	if (abs_x < (1./0.35)) \
	{ \
		R = ra_f##BITS[0]+s*(ra_f##BITS[1]+s*(ra_f##BITS[2]+s*(ra_f##BITS[3]+s*(ra_f##BITS[4]+s*(ra_f##BITS[5]+s*(ra_f##BITS[6]+s*ra_f##BITS[7])))))); \
		S = sa_f##BITS[0]+s*(sa_f##BITS[1]+s*(sa_f##BITS[2]+s*(sa_f##BITS[3]+s*(sa_f##BITS[4]+s*(sa_f##BITS[5]+s*(sa_f##BITS[6]+s*(sa_f##BITS[7]+s*sa_f##BITS[8]))))))); \
	} \
	else /* |x| > 1/.35 */ \
	{ \
		R = rb_f##BITS[0]+s*(rb_f##BITS[1]+s*(rb_f##BITS[2]+s*(rb_f##BITS[3]+s*(rb_f##BITS[4]+s*(rb_f##BITS[5]+s*rb_f##BITS[6]))))); \
		S = sb_f##BITS[0]+s*(sb_f##BITS[1]+s*(sb_f##BITS[2]+s*(sb_f##BITS[3]+s*(sb_f##BITS[4]+s*(sb_f##BITS[5]+s*(sb_f##BITS[6]+s*sb_f##BITS[7])))))); \
	} \
	z.value_float = abs_x; \
...
{ \
	...
	{ \
		if (abs_x < 0x1p-56) \
		{ \
			return (0.125 * (8 * x + efx8_f##BITS * x));	/* avoid underflow */ \
		} \
		z = x*x; \
		r = pp_f##BITS[0]+z*(pp_f##BITS[1]+z*(pp_f##BITS[2]+z*(pp_f##BITS[3]+z*pp_f##BITS[4]))); \
		s = qq_f##BITS[0]+z*(qq_f##BITS[1]+z*(qq_f##BITS[2]+z*(qq_f##BITS[3]+z*(qq_f##BITS[4]+z*qq_f##BITS[5])))); \
		y = r/s; \
		return (x + x*y); \
	} \
	if (abs_x < 6.) \
...
static const t_f32
erx_f32  = +8.4506291151e-01, /* 0x3f58560b */
/* Coefficients for approximation to  erf on [0,0.84375] */
efx8_f32 = +1.0270333290e+00, /* 0x3f8375d4 */
pp_f32[] = {
...
static const t_f64
erx_f64  = 8.45062911510467529297e-01, /* 0x3FEB0AC1, 0x60000000 */
/* Coefficients for approximation to  erf on [0,0.84375] */
efx8_f64 = +1.02703333676410069053e+00, /* 0x3FF06EBA, 0x8214DB69 */
pp_f64[] = {
...
static const t_f80
erx_f80 = 0.845062911510467529296875L,
/* Coefficients for approximation to  erf on [0,0.84375] */
/* 8 * (2/sqrt(pi) - 1) */
efx8_f80 = 1.0270333367641005911692712249723613735048e+00L,
pp_f80[] = {
