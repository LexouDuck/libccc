...
const struct exp2_data
{
	...
	t_f64	exp2_shift;
	t_f64	exp2_poly[EXP2_POLY_ORDER];
	t_u64	table[2*N_F64];
}	__exp2_data;

const struct exp2_data __exp2_data =
{
	...
	.table = {
		// 2^(k/N) ~= H[k]*(1 + __exp2_data.table[k]) for int k in [0,N)
		// table[2*k] = AS_U64(__exp2_data.table[k])
...
static const struct exp2l_data __exp2l_data =
{
...
t_f32	F32_Exp2(t_f32 x)
{
	...
	kd = xd + __exp2f_data.shift_scaled;
	ki = AS_U64(kd);
	kd -= __exp2f_data.shift_scaled; /* k/N for int k. */
	r = xd - kd;
	/* exp2(x) = 2^(k/N) * 2^r ~= s * (C0*r^3 + C1*r^2 + C2*r + 1) */
	t = __exp2f_data.table[ki % N_F32];
	t += ki << (52 - EXP2F_TABLE_BITS);
	s = AS_F64(t);
	z = __exp2f_data.poly[0] * r + __exp2f_data.poly[1];
	r2 = r * r;
	y = __exp2f_data.poly[2] * r + 1;
	y = z * r2 + y;
	y = y * s;
	return (t_f32)(y);
}
...
t_f64	F64_Exp2(t_f64 x)
{
	...
	kd = (x + __exp2_data.exp2_shift);
	ki = AS_U64(kd); /* k. */
	kd -= __exp2_data.exp2_shift; /* k/N for int k. */
	r = x - kd;
	/* 2^(k/N) ~= scale * (1 + tail). */
	idx = 2 * (ki % N_F64);
	top = ki << (52 - EXP_TABLE_BITS);
	tail = AS_F64(__exp2_data.table[idx]);
	/* This is only a valid scale when -1023*N < k < 1024*N. */
	sbits = __exp2_data.table[idx + 1] + top;
	/* exp2(x) = 2^(k/N) * 2^r ~= scale + scale * (tail + 2^r - 1). */
	/* Evaluation is optimized assuming superscalar pipelined execution. */
	r2 = r * r;
	/* Without fma the worst case error is 0.5/N ulp larger. */
	/* Worst case error is less than 0.5+0.86/N+(abs poly error * 2^53) ulp. */
	tmp = tail + r * __exp2_data.exp2_poly[0] + r2 * (__exp2_data.exp2_poly[1] + r * __exp2_data.exp2_poly[2]) + r2 * r2 * (__exp2_data.exp2_poly[3] + r * __exp2_data.exp2_poly[4]);
...
t_f80	F80_Exp2(t_f80 x)
{
	...
	*/
	u.f = x + __exp2l_data.redux;
	i0 = u.i.m + N_F80 / 2;
	k.u = i0 / N_F80 * N_F80;
	k.i /= N_F80;
	i0 %= N_F80;
	u.f -= __exp2l_data.redux;
	z = x - u.f;
	...
	r = tbl_lo+(tbl_hi+tbl_lo)*
		z*(__exp2l_data.poly[1]+
		z*(__exp2l_data.poly[2]+
		z*(__exp2l_data.poly[3]+
		z*(__exp2l_data.poly[4]+
		z*(__exp2l_data.poly[5]+
		z*(__exp2l_data.poly[6])
		)))))+tbl_hi;
...
t_f128	F128_Exp2(t_f128 x)
{
	...
	*/
	...
	t = __exp2l_data.table[i0];
	z -= __exp2l_data.eps[i0];
	r = t+t*
		z*(__exp2l_data.poly[1]+
		z*(__exp2l_data.poly[2]+
		z*(__exp2l_data.poly[3]+
		...
		z*(__exp2l_data.poly[5]+
		z*(__exp2l_data.poly[6]+
		z*(__exp2l_data.poly[7]+
		z*(__exp2l_data.poly[8]+
		z*(__exp2l_data.poly[9]+
		z*(__exp2l_data.poly[10])
		)))))))));
	return scalbnl(r, k.i);
}
#endif
