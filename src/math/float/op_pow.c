...
__data_pow_f32 =
{
	.tab =
	{
		...
		{ +0x1.49539F0F010B0p+0, -0x1.7418B0A1FB77Bp-2 * POWF_SCALE },
		{ +0x1.3C995B0B80385p+0, -0x1.39DE91A6DCF7Bp-2 * POWF_SCALE },
		{ +0x1.30D190C8864A5p+0, -0x1.01D9BF3F2B631p-2 * POWF_SCALE },
		{ +0x1.25E227B0B8EA0p+0, -0x1.97C1D1B3B7AF0p-3 * POWF_SCALE },
		{ +0x1.1BB4A4A1A343Fp+0, -0x1.2F9E393AF3C9Fp-3 * POWF_SCALE },
		{ +0x1.12358F08AE5BAp+0, -0x1.960CBBF788D5Cp-4 * POWF_SCALE },
		{ +0x1.0953F419900A7p+0, -0x1.A6F9DB6475FCEp-5 * POWF_SCALE },
		{ +0x1.0000000000000p+0, +0x0.0000000000000p+0 * POWF_SCALE },
		{ +0x1.E608CFD9A47ACp-1, +0x1.338CA9F24F53Dp-4 * POWF_SCALE },
		{ +0x1.CA4B31F026AA0p-1, +0x1.476A9543891BAp-3 * POWF_SCALE },
	...
	.poly =
	{
		...
		+0x1.71547652AB82Bp+0 * POWF_SCALE,
	}
};
...
t_f64 F32_log2_inline(t_u32 ix)
{
	...
	invc = __data_pow_f32.tab[i].invc;
	logc = __data_pow_f32.tab[i].logc;
	z = (t_f64)AS_F32(iz);
...
*/
static inline
float F32_exp2_inline(t_f64 xd, t_u32 sign_bias)
{
	t_u64 ki, ski, t;
	t_f64 kd, z, r, r2, y, s;
...
t_f32	F32_Pow(t_f32 x, t_f32 y)
{
	...
	if (predict_false((AS_U64(ylogx) >> 47 & 0xffff) >= AS_U64(126.0 * POWF_SCALE) >> 47))
	{
		/* |y*log(x)| >= 126. */
...
struct data_pow_f64
{
	...
	struct { t_f64 invc, pad, logc, logctail; } tab[N];
}
__data_pow_f64 =
{
...
and z falls into the ith one, then table entries are computed as
	...
	tab[i].logctail = (t_f64)(log(c) - logc)
...
t_f64 log_inline(t_u64 ix, t_f64 *tail)
{
	...
	*/
	...
	invc = __data_pow_f64.tab[i].invc;
	logc = __data_pow_f64.tab[i].logc;
	logctail = __data_pow_f64.tab[i].logctail;
...
**	(int32_t)KI is the k used in the argument reduction and exponent adjustment of scale, positive k here means the result may overflow and negative k means the result may underflow.
*/
static inline
t_f64 F64_Pow_specialcase(t_f64 tmp, t_u64 sbits, t_u64 ki)
{
	...
	{
		/*
		Round y to the right precision before scaling it into the subnormal range to avoid double rounding
...
__data_pow_f80 =
{
	...
	/* antilog[i] = 2^(-i/32), rounded to IEEE long double precision. */
...
{ \
 ...
 \
	...
	*/ \
	z = Hb * __polevll(Hb, __data_pow_f80.R, 6);  /*  z = 2**Hb - 1 */ \
	/* \
