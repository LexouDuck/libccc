...
inline
t_u32	c_random_32bit_shuffle(t_u32 n)
{
	n = (n << 12) ^ (n >> 20);
...
inline
void		c_random_set_seed(t_u32 seed)
{
	random_U32 = seed;
}
...
inline
t_u32	c_random_u32_0_to_pow2n(t_u32 n)
{
	return (c_random_u32() & ((0x1 << n) - 1));
}

inline
t_u32	c_random_u32_0_to_n(t_u32 n)
{
	return (n == 0 ? n : (c_random_u32() % n));
}

inline
t_sint	c_random_int_a_to_b(t_sint a, t_sint b)
{
	if (a < b)
...
inline
t_float	c_random_float_0_to_1(void)
{
	return (((t_float)c_random_u32()) / (t_float)MODULUS);
}

inline
t_float	c_random_float_a_to_b(t_float a, t_float b)
{
	if (a < b)
