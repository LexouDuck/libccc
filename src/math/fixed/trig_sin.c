...
#define DEFINEFUNC_FIXED_SIN(BITS) \
t_q##BITS	Q##BITS##_Sin(t_q##BITS x) \
{ \
	if CCCERROR((Q##BITS##_IsNaN(x)), ERROR_NANARGUMENT, NULL) \
		return (Q##BITS##_ERROR); \
	if CCCERROR((Q##BITS##_IsInf(x)), ERROR_MATHDOMAIN, \
		"sine of infinite value is undefined") \
		return (Q##BITS##_ERROR); \
	return (Q##BITS##_FromF64(F64_Sin(F64_FromQ##BITS(x)))); \
} \

DEFINEFUNC_FIXED_SIN(8)
DEFINEFUNC_FIXED_SIN(16)
DEFINEFUNC_FIXED_SIN(32)
DEFINEFUNC_FIXED_SIN(64)
#if LIBCONFIG_USE_INT128
DEFINEFUNC_FIXED_SIN(128)
#endif
