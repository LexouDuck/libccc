...
#define DEFINEFUNC_FIXED_TAN(BITS) \
t_q##BITS	Q##BITS##_Tan(t_q##BITS x) \
{ \
	if CCCERROR((Q##BITS##_IsNaN(x)), ERROR_NANARGUMENT, NULL) \
		return (Q##BITS##_ERROR); \
	if CCCERROR((Q##BITS##_IsInf(x)), ERROR_MATHDOMAIN, \
		"tangent of infinite value is undefined") \
		return (Q##BITS##_ERROR); \
	return (Q##BITS##_FromF64(F64_Tan(F64_FromQ##BITS(x)))); \
} \

DEFINEFUNC_FIXED_TAN(8)
DEFINEFUNC_FIXED_TAN(16)
DEFINEFUNC_FIXED_TAN(32)
DEFINEFUNC_FIXED_TAN(64)
#if LIBCONFIG_USE_INT128
DEFINEFUNC_FIXED_TAN(128)
#endif
