...
/*
**	The base-10 logarithm is computed with integer-only arithmetic, via the
**	base-2 logarithm core: `log10(x) = log2(x) * log10(2)`, carried as a
**	signed q54 value, then scaled by `DENOM` (truncated toward zero, like all
**	fixed-point results).
*/

#define DEFINEFUNC_FIXED_LOG10(BITS, WB) \
t_q##BITS	Q##BITS##_Log10(t_q##BITS x) \
{ \
	...
	if CCCERROR((x._ < 0), ERROR_MATHDOMAIN, \
		...
		return (Q##BITS##_ERROR); \
	if (x._ == 0) \
		return (Q##BITS##_MIN); /* log10(0) == -INFINITY */ \
	if (Q##BITS##_IsInf(x)) \
		return (Q##BITS##_MAX); \
	...
	t_u64	c_hi; \
	t_u64	c_lo; \
	__fixedmath_umul_64(abs_l0, (t_u64)__FIXEDMATH_LOG10_2_Q62, &c_hi, &c_lo); \
	...
	if CCCERROR(((hi >> 54) != 0 || q > (t_u##WB)(t_u##BITS)Q##BITS##_MAX._), ERROR_RESULTRANGE, \
		"%s overflow for fixed-point base-10 logarithm (as t_q"#BITS")", (negative ? "negative" : "positive")) \
	{ \
...
DEFINEFUNC_FIXED_LOG10(8, 64)
DEFINEFUNC_FIXED_LOG10(16, 64)
DEFINEFUNC_FIXED_LOG10(32, 64)
DEFINEFUNC_FIXED_LOG10(64, 64)
#if LIBCONFIG_USE_INT128
DEFINEFUNC_FIXED_LOG10(128, 128)
#endif
