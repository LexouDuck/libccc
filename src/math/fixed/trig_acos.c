...
#define DEFINEFUNC_FIXED_ARCCOS(BITS) \
t_q##BITS	Q##BITS##_ArcCos(t_q##BITS x) \
{ \
	...
	if CCCERROR((Q##BITS##_Abs(x)._ > (t_s##BITS)Q##BITS##_DENOM), ERROR_MATHDOMAIN, \
		"arc-cosine is only defined in the interval [-1, +1]") \
		return (Q##BITS##_ERROR); \
	return (Q##BITS##_FromF64(F64_ArcCos(F64_FromQ##BITS(x)))); \
} \

DEFINEFUNC_FIXED_ARCCOS(8)
DEFINEFUNC_FIXED_ARCCOS(16)
DEFINEFUNC_FIXED_ARCCOS(32)
DEFINEFUNC_FIXED_ARCCOS(64)
#if LIBCONFIG_USE_INT128
DEFINEFUNC_FIXED_ARCCOS(128)
#endif
