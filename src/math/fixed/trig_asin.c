...
#define DEFINEFUNC_FIXED_ARCSIN(BITS) \
t_q##BITS	Q##BITS##_ArcSin(t_q##BITS x) \
{ \
	...
	if CCCERROR((Q##BITS##_Abs(x)._ > (t_s##BITS)Q##BITS##_DENOM), ERROR_MATHDOMAIN, \
		"arc-sine is only defined in the interval [-1, +1]") \
		return (Q##BITS##_ERROR); \
	return (Q##BITS##_FromF64(F64_ArcSin(F64_FromQ##BITS(x)))); \
} \

DEFINEFUNC_FIXED_ARCSIN(8)
DEFINEFUNC_FIXED_ARCSIN(16)
DEFINEFUNC_FIXED_ARCSIN(32)
DEFINEFUNC_FIXED_ARCSIN(64)
#if LIBCONFIG_USE_INT128
DEFINEFUNC_FIXED_ARCSIN(128)
#endif
