...
#define DEFINEFUNC_FIXED_COS(BITS) \
t_q##BITS	Q##BITS##_Cos(t_q##BITS x) \
{ \
	if CCCERROR((Q##BITS##_IsNaN(x)), ERROR_NANARGUMENT, NULL) \
		return (Q##BITS##_ERROR); \
	if CCCERROR((Q##BITS##_IsInf(x)), ERROR_MATHDOMAIN, \
		"cosine of infinite value is undefined") \
		return (Q##BITS##_ERROR); \
	return (Q##BITS##_FromF64(F64_Cos(F64_FromQ##BITS(x)))); \
} \

DEFINEFUNC_FIXED_COS(8)
DEFINEFUNC_FIXED_COS(16)
DEFINEFUNC_FIXED_COS(32)
DEFINEFUNC_FIXED_COS(64)
#if LIBCONFIG_USE_INT128
DEFINEFUNC_FIXED_COS(128)
#endif
