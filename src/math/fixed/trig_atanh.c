...
#define DEFINEFUNC_FIXED_INVTANH(BITS) \
t_q##BITS	Q##BITS##_InvTanH(t_q##BITS x) \
{ \
	if CCCERROR((Q##BITS##_IsNaN(x)), ERROR_NANARGUMENT, NULL) \
		return (Q##BITS##_ERROR); \
	if CCCERROR((Q##BITS##_Abs(x)._ > (t_s##BITS)Q##BITS##_DENOM), ERROR_MATHDOMAIN, \
		"inverse hyperbolic tangent is only defined in the interval [-1, +1]") \
		return (Q##BITS##_ERROR); \
	if (x._ == +(t_s##BITS)Q##BITS##_DENOM) \
		return (Q##BITS##_MAX); /* atanh(+1) == +INFINITY */ \
	if (x._ == -(t_s##BITS)Q##BITS##_DENOM) \
		return (Q##BITS##_MIN); /* atanh(-1) == -INFINITY */ \
	return (Q##BITS##_FromF64(F64_InvTanH(F64_FromQ##BITS(x)))); \
} \

DEFINEFUNC_FIXED_INVTANH(8)
DEFINEFUNC_FIXED_INVTANH(16)
DEFINEFUNC_FIXED_INVTANH(32)
DEFINEFUNC_FIXED_INVTANH(64)
#if LIBCONFIG_USE_INT128
DEFINEFUNC_FIXED_INVTANH(128)
#endif
