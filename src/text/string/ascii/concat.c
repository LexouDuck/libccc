...
t_ascii*		StringASCII_Concat(t_ascii const* str1, t_ascii const* str2)
{
	...
	if CCCERROR((str2 == NULL), ERROR_NULLPOINTER, "right-hand-side string given is NULL")
		return (NULL);
	length1 = StringASCII_Length(str1);
	length2 = StringASCII_Length(str2);
...
t_ascii*	StringASCII_Append(t_ascii** a_dest, t_ascii const* src)
{
	t_ascii*	tmp;
	...
	if CCCERROR((src == NULL), ERROR_NULLPOINTER, "string to append given is NULL")
		return (NULL);
	tmp = StringASCII_Concat(*a_dest, src);
...
t_ascii*	StringASCII_Prepend(t_ascii const* src, t_ascii** a_dest)
{
	t_ascii*	tmp;
	...
	if CCCERROR((src == NULL), ERROR_NULLPOINTER, "string to prepend given is NULL")
		return (NULL);
	tmp = StringASCII_Concat(src, *a_dest);
	StringASCII_Delete(a_dest);
	*a_dest = tmp;
	return (*a_dest);
}
...
t_ascii*	StringASCII_Merge(t_ascii** a_str1, t_ascii** a_str2)
{
	...
	if CCCERROR((a_str2 == NULL), ERROR_NULLPOINTER, "right-hand-side string given is NULL")
		return (NULL);
	result = StringASCII_Concat(*a_str1, *a_str2);
	StringASCII_Delete(a_str1);
	StringASCII_Delete(a_str2);
	*a_str1 = result;
	*a_str2 = result;
	return (result);
}
