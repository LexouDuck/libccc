...
//	int		ft_tolower(int c);
	...
	printf("\n==> "C_BLUE"Ideally, all of these char function tests should succeed, and no output will be shown."RESET);
	printf("\n==> "C_BLUE"(You may see warnings for characters beyond 255, those chars are undefined behavior)."RESET);
	printf("\n==> "C_BLUE"In any case, errors/warnings will be displayed if there are any, important or not."RESET"\n");
...
printf("\n");
	...
	if (error || warning)
		printf("\nA total of "C_RED"%d"RESET" errors and "C_YELLOW"%d"RESET" warnings found, testing every char up to %d\n", error, warning, charmax);
