...
{																					\
	...
	{																				\
		...
		HANDLE_ERROR_SF(RESULTRANGE, (tmp < result),								\
			...
			" ("#BITS"-bit unsigned integer overflow for \"%s\" at "SF_U##BITS##_OCT")", str, U##BITS##_MAX)	\
		result = tmp;																\
	}																				\
	if (dest)	*dest = result;														\
	return (i);																		\
...
{																					\
	...
	{																				\
		...
		HANDLE_ERROR_SF(RESULTRANGE, (negative && tmp > (t_u##BITS)S##BITS##_MIN),	\
			...
			" ("#BITS"-bit signed integer underflow for \"%s\" at "SF_S##BITS##_OCT")", str, S##BITS##_MIN)	\
		HANDLE_ERROR_SF(RESULTRANGE, (!negative && tmp > (t_u##BITS)S##BITS##_MAX),	\
			LIBCONFIG_ERROR_PARSEROVERFLOW(S##BITS##_MAX),							\
			" ("#BITS"-bit signed integer overflow for \"%s\" at "SF_S##BITS##_OCT")", str, S##BITS##_MAX)	\
		result = tmp;																\
	}																				\
