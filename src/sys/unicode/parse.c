...
t_size		UTF32_Parse_N(t_utf32* dest, t_ascii const* str, t_size n)
{
	...
	else if (str[i] == 'u')
	{
		...
		if (((code1 >= UTF16_SURROGATE_LO) && (code1 < UTF16_SURROGATE_END)))
			LIBCONFIG_HANDLE_PARSINGERROR(ERROR, PARSINGERROR_UTF16"invalid UTF-16 char code ("SF_HEX_U16")", code1)
		...
		{	// UTF16 surrogate pair
			...
			if ((code2 < UTF16_SURROGATE_LO) || (code2 >= UTF16_SURROGATE_END))
				LIBCONFIG_HANDLE_PARSINGERROR(ERROR, PARSINGERROR_UTF16_SURROGATE"invalid UTF-16 char code for second half of the surrogate pair ("SF_HEX_U16")", code2)
