...
static
t_char	String_Parse_GetEscape(t_char escapechar)
{
	static const struct { t_char esc; t_char chr; } lookuptable[] =
	{
	...
	while (index < n && str[index])
	{
		if (str[index] == '\\') // escape sequence
		{
			...
			HANDLE_ERROR_SF(PARSE, (index == n || str[index] == '\0'), return (0);,
				"string ends with backslash, potential buffer overrun:\n%s", str)
			t_char	escapechar = String_Parse_GetEscape(str[index]);
			if (escapechar != ERROR)
				result[i++] = escapechar;
