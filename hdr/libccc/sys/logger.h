...
_FORMAT(printf, 5, 6)
e_stderror 	Log(s_logger const* logger,
...
e_stderror				Log_Fatal	(s_logger const* logger, t_char const* str);
...
_FORMAT(printf, 3, 4)
e_stderror					LogAll_Error_IO			(t_logptrarr const loggers, int error_code, t_char const* format_str, ...);
#define c_logall_error_io	LogAll_Error_IO
#define LogAll_SystemError	LogAll_Error_IO

//! Logging perror-style to both stderr and logfile (if applicable)
_FORMAT(printf, 3, 4)
e_stderror					LogAll_Error			(t_logptrarr const loggers, int error_code, t_char const* format_str, ...);
#define c_logall_error		LogAll_Error

//! To be called when there is an important warning to show to the user
_FORMAT(printf, 2, 3)
e_stderror					LogAll_Warning			(t_logptrarr const loggers,                 t_char const* format_str, ...);
...
_FORMAT(printf, 2, 3)
e_stderror					LogAll_Success			(t_logptrarr const loggers,                 t_char const* format_str, ...);
#define c_logall_success	LogAll_Success

//! Logging printf-style of message to both stdout and logfile (if applicable)
_FORMAT(printf, 2, 3)
e_stderror					LogAll_Message			(t_logptrarr const loggers,                 t_char const* format_str, ...);
#define c_logall_message	LogAll_Message

//! Logging printf-style of verbose message to both stdout and logfile (if applicable)
_FORMAT(printf, 2, 3)
e_stderror					LogAll_Message_Verbose	(t_logptrarr const loggers,                 t_char const* format_str, ...);
...
//! Logging (perror-style) to both stderr and logfile (if applicable)
_FORMAT(printf, 3, 4)
e_stderror					Log_Error_IO			(s_logger const* logger, int error_code, t_char const* format_str, ...);
#define c_log_error_io	Log_Error_IO
#define Log_SystemError	Log_Error_IO

//! Logging perror-style to both stderr and logfile (if applicable)
_FORMAT(printf, 3, 4)
e_stderror					Log_Error				(s_logger const* logger, int error_code, t_char const* format_str, ...);
#define c_log_error		Log_Error

//! To be called when there is an important warning to show to the user
_FORMAT(printf, 2, 3)
e_stderror					Log_Warning				(s_logger const* logger,                 t_char const* format_str, ...);
#define c_log_warning	Log_Warning

//! To be called when there is an successful operation (or result) of which to notify the user
_FORMAT(printf, 2, 3)
e_stderror					Log_Success				(s_logger const* logger,                 t_char const* format_str, ...);
#define c_log_success	Log_Success

//! Logging printf-style of message to both stdout and logfile (if applicable)
_FORMAT(printf, 2, 3)
e_stderror					Log_Message				(s_logger const* logger,                 t_char const* format_str, ...);
#define c_log_message	Log_Message

//! Logging printf-style of verbose message to both stdout and logfile (if applicable)
_FORMAT(printf, 2, 3)
e_stderror					Log_Message_Verbose		(s_logger const* logger,                 t_char const* format_str, ...);
