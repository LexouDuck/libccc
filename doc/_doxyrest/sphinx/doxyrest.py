...
def create_ref_node(raw_text, text, target, language=None):
    if not target:
...
def create_target_node(raw_text, text, target, language, lineno, document, extra_classes=[]):
...
class RefCodeBlock(Directive):
    ...
    def run(self):
        ...
        global_config_language = config.highlight_language
        ...
        while True:
            ...
            if role == ':target:':
                ...
                new_node = create_ref_node(raw_text, text, target, language)

            node += new_node
#            warnings.warn('node: ' + new_node.pformat(), Warning, 2)
    ...
    def apply(self):
        for node in self.document.traverse(RefTransform.node_filter):
            ...
            while True:
                ...
                node += create_ref_node(raw_text, text, target, global_config_language)
                pos = match.end()
